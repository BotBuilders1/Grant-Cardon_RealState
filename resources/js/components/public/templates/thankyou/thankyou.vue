...
<div class="template">
	<div class="row" style="background: #1c1c1c; margin-top:0;width:100%;position:fixed;top:0;z-index:20;">
		...
		<div class="col-sm-6  no-padding-tb">
			<div class="mobile-header-container" style="width:520px;margin: 0 auto;display: block;height: 55px;">
				<img class="mobile-header-label" style="float:left;width: 40px;margin-top: 10px;" src="/image/ty/red_check.png" >
				<label class="mobile-schedule" style="color:#ffffff;font-size:25px;font-weight:700;font-family:sans-serif;margin: 10px 0 0 5px;display: block;float: left;">Confirmed for </label>
				<label class="mobile-schedule" style="color:#d09b2f;font-size:25px;font-weight:700;font-family:sans-serif;margin: 10px 0 0 5px;display: block;float: left;">
					{{thankYouData.date}}
				</label>
			</div>
	...
	<div class="container container-body mt-30 mobile-fixed-header" style="margin-top:155px;">
		<div class="row">
			<div class="col-sm-12 mb-30 ">
				<img src="/image/ty/title2.png" class="top-sticker" style="width:85%;display:block;margin:45px auto;" />
				<iframe src="https://player.vimeo.com/video/542849609?autoplay=1" class="videoframe" width="100%" frameborder="0" allow="autoplay; fullscreen" allowfullscreen></iframe>
			</div>
		</div>
		<div class="row">

			<div class="col-sm-6">
				<div class="mid-container mobile-mid-container">
					...
					<div class="row">
						...
						<div class="col-sm-8 pt0">
							...
							<template v-if="hasLogedInFb">
								<div id="ditoEmbed" class="mcwidget-embed" data-widget-id="15503297" :data-widget-payload="thankYouData.user_id" style="margin-top:20px;margin-bottom:20px;"></div>
							</template>
							<template v-else>
								<a @click="downloadFunc()">
			...
			<div class="col-sm-6">
				<div class="mid-container mobile-mid-container">
					...
					<div class="row">
						...
						<div class="col-sm-12">
							<template v-if="thankYouData && thankYouData.jot != 1">
								<div class="middle-button-container" style="text-align:center;margin-top:15px;margin-bottom:16px;">
									<b-dropdown size="lg" variant="grey" toggle-class="" no-caret class="mobile-version-calendar">
								    	<template v-slot:button-content> 
								    		<img src="/image/ty/calendar-icon.png" class="notjot-icon"> ADD TO CALENDAR
							    		</template>
									    <b-dropdown-item v-bind:href="calendarUrl('apple')" >Apple Calendar</b-dropdown-item>
									    <b-dropdown-item v-bind:href="calendarUrl('google')" target="_blank" >Google <em>(online)</em></b-dropdown-item>
									    <b-dropdown-item v-bind:href="calendarUrl('outlook')">Outlook</b-dropdown-item>
									    <b-dropdown-item v-bind:href="calendarUrl('outlookcom')" target="_blank" >Outlook.com <em>(online)</em></b-dropdown-item>
									    <b-dropdown-item v-bind:href="calendarUrl('yahoo')" target="_blank" >Yahoo <em>(online)</em></b-dropdown-item>
									</b-dropdown>

									<b-btn v-bind:href="thankYouData.live_room_url" target="_blank" variant="blue">
										<img src="/image/ty/laptop-icon.png" class="notjot-icon2"> TRAINING LINK
									</b-btn>
								</div>
							</template>
							<template v-else>
		...
		<div class="row mobile-section " style="display:none;margin-bottom:50px;">
			...
			<div class="col-sm-12">
				...
				<template v-if="thankYouData && thankYouData.jot != 1">
					<div class="middle-button-container" style="text-align:center;margin-top:20px;">
						<b-dropdown size="lg" variant="grey" toggle-class="" no-caret class="mobile-version-calendar">
					    	<template v-slot:button-content> 
					    		<img src="/image/ty/calendar-icon.png" class="notjot-icon"> ADD TO CALENDAR
				    		</template>
		...
		<div class="container"  style="background-color:#212529">
			<div class="row justify-content-md-center">
				<div class="col-sm-12 mobile-no-padding-top">
					<img class="mobile-footer-logo" src="/image/ty/gc-logo.png" style="display: block;margin: 30px auto 0;width: 25%;">
				</div>
...
@media (min-width:1px) and (max-width: 640px) {
	...
	.mobile-schedule{font-size:16px !important;}
	.mobile-fixed-header{margin-top:190px !important;}
...
.notjot-icon{ width: 20px;margin-top:-2px; }
.notjot-icon2{ width:20px;margin-top:-4px; }
...
export default {
	...
	methods: {
		...
		calendarUrl (type) {
			...
			url += "&title=💰 10X Business Workshop with Grant Cardone"
		...
		trainingLink (){
			window.open(this.thankYouData.live_room_url);
		},
		downloadFunc(){
			window.open("https://10xworkshop.com/api/workbook/download");

			// window.open("https://10xworkshop.com/api/businessplan/workbook/complete");
			// window.open("https://10xworkshop.com/api/businessplan/workbook/customizable");
		}
    ...
    mounted() {
				...
				if (response.status === 'connected' || response.status === 'not_authorized') {
					...
					setTimeout(function(){
						window.mcAsyncInit = function() {
							const widgetInstanceElement = document.getElementById('ditoEmbed')
							const widget = MC.getWidget(widgetInstanceElement)
							widget.setPayload(app.thankYouData.user_id)
						}
					},500)
		...
		**/
		...
		setTimeout(()=>{
			gtag('event', 'conversion', {'send_to': 'AW-743408467/csCOCPT7kM0CENOGvuIC'});
		}, 1000);
